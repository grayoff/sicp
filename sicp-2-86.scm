...
(define (install-integer-package)
  ...
  (put 'atang '(integer) (lambda (x) (make-real (atan x))))
...
(define (install-rational-package)
  ...
  (put 'atang '(rational) (lambda (x) (make-real (atan (/ (numer x) (denom x))))))
...
(define (install-real-package)
  ...
  (put 'raise '(real) (lambda (x) (make-complex-from-real-imag (make-real x) (make-real 0))))
  ...
  (put 'atang '(real) (lambda (x) (tag (atan x))))
...
(define (install-rectangular-package)
  ...
  (define (magnitude z)
    (sqroot (add (square (real-part z))
             (square (imag-part z)))))
  (define (angle z)
    (atang (div (imag-part z) (real-part z))))
...
(drop c)
(drop d)
; (drop (make-complex-from-real-imag 3.25 0.0))  ; not allowed anymore
(add d d)
(add a d)
"---"
(sine a)
...
; need to implement sine and cosine for all numbers below real, or implement rising to higher levels in search for op

(define e (make-complex-from-real-imag (make-rational 13 4) (make-integer 2)))
e
(real-part e)
(imag-part e)
;(project e) // no proper project implementation
